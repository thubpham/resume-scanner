...
from agent_manager import AgentManager, EmbeddingManager
...
class ScoreImprovementService:
    ...
    async def improve_score_with_llm(
        ...
        # --- UPDATED: Prompt for resume improvement is changed in 'resume_improvement.py' ---
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            # Embedding of the improved resume
            emb = await self.embedding_manager.embed(text=improved)
            job_emb = await self.embedding_manager.embed(text=job)
    ...
    async def run(self, resume_id: str, job_id: str) -> Dict:
        ...
        # --- UPDATED: run task for job embedding, not 
        # job_kw_embedding_task = asyncio.create_task(
        #     self.embedding_manager.embed(extracted_job_keywords)
        # )

        job_embedding_task = asyncio.create_task(
            self.embedding_manager.embed(job.content)
        )

        resume_embedding, job_embedding = await asyncio.gather(
            resume_embedding_task, job_embedding_task
        )
    ...
    async def run_and_stream(self, resume_id: str, job_id: str) -> AsyncGenerator:
        ...
        job_embedding = await self.embedding_manager.embed(text=job.content)
